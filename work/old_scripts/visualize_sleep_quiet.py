...
sleep.resample('ME').agg({"durationHr": ['mean', 'sem']}) 

outSleep = sleep.resample('ME').agg({"durationHr": ['mean', 'sem']}) 

export_csv = outSleep.to_csv(out, index=True, header=True, sep=';')

rest = pd.read_csv(out, delimiter=';', decimal=",",
                   names=["date", "mean", "sem"],
                   parse_dates=["date"],
                   date_format="%Y-%m-%d %H:%M:%S",  # ajout format explicite
                   index_col="date")
...
resultSleep = pd.concat([rest, bed_failure], axis=1).reindex(rest.index)
