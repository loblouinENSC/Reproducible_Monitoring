...
sub breakfast() {
  my ($Ts,$Te) = @_;
  sub{my $x=&slot_2017($Ts,$Te); [&and, &occ("ContactS_Cupboard",$x), &occ(&up("EMeter_Coffeemaker"),$x)]}->();
}
sub lunch() {
  my ($Ts,$Te,$T) = @_;
  sub{&occ([&and, &recent(&up("ContactS_Fridge"),$T), "EMeter_Coffeemaker"],&slot_2017($Ts,$Te))}->();
}
sub dinner() {
  my ($Ts,$Te) = @_;
  sub{my $x=&slot_2017($Ts,$Te); [&and, &occ(&up("ContactS_Fridge"),$x), &occ(&up("EMeter_L"),$x)]}->();
}
...
sub{$sleep_quiet}->(),
"breakfast_7_9",
sub{&breakfast(18000000,36000000)}->(),
"lunch_12_14",
sub{&lunch(39600000,54000000,1500000)}->(),
"dinner_18_21",
sub{&dinner(64800000,82800000)}->(),
]
)
